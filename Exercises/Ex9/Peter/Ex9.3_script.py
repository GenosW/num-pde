...
def primal(mesh, order=2, draw=False, rt=False):
    ...
    gfup.Set(ud,
             BND)  # BND =^ definedon=mesh.Boundaries("bottom|right|top|left")
...
def mixed(mesh, order_flux=2, draw=False, rt=False):
    ...
    V = HDiv(mesh, order=order_flux,
             dirichlet=Neumann)  # "Neumann is the new Dirichlet"
    ...
    am += (1 / alpha * (tau.Trace() * normal) * (sigma.Trace() * normal)) * ds(definedon=mesh.Boundaries(Robin))
    am.Assemble()

    fm = LinearForm(FESm)
    #     source  + linear Dirichlet-term                                            + linear Robin-term
    fm += -f * v * dx + ud * (tau.Trace() * normal) * ds(definedon=mesh.Boundaries(Dirichlet)) 
    fm += g / alpha * (tau.Trace() * normal) * ds(definedon=mesh.Boundaries(Robin))
    fm.Assemble()
...
for order, style in zip(orders, styles):
    ...
    plt.semilogy(hs_used,
                 errors_flux,
                 "--" + style,
                 label=f"err(flux)_o={order}")
plt.grid()
plt.legend()
